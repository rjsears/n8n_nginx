...
### 13. Public Website (`public_website`)

Optional public website hosting configuration.

| Variable | Description | Default | Required | Sensitive |
|----------|-------------|---------|----------|-----------|
| `PUBLIC_SITE_ENABLE` | Enable public website hosting (www subdomain) | `false` | No | No |
| `PUBLIC_WEBSITE_DOMAIN` | Domain for public website | `www.${ROOT_DOMAIN}` | No | No |

**Note:** When enabled, an additional `nginx_router` container is created for hostname-based routing, and `nginx_public` serves static files.

**Example:**
```bash
PUBLIC_SITE_ENABLE=true
PUBLIC_WEBSITE_DOMAIN=www.example.com
```
