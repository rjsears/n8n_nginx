...
prompt_with_default() {
    ...
    if [ "$is_password" = "true" ]; then
        echo -ne "${WHITE}  $prompt ${GRAY}[hidden]${NC}: "
        read -s value
        echo ""
    else
        echo -ne "${WHITE}  $prompt ${GRAY}[$default]${NC}: "
        read value
    fi
...
confirm_prompt() {
    ...
    if [ "$default" = "y" ]; then
        echo -ne "${WHITE}  $prompt ${GRAY}[Y/n]${NC}: "
    else
        echo -ne "${WHITE}  $prompt ${GRAY}[y/N]${NC}: "
    fi
...
perform_system_checks() {
    ...
    if [ "$available_space" -ge 5 ]; then
        ...
        print_warning "Disk space: ${available_space}GB available (5GB recommended)"
        all_checks_passed=false
    fi

    # Check available memory (recommend at least 2GB)
    local total_memory=$(free -g 2>/dev/null | awk '/^Mem:/{print $2}' || echo "unknown")
    if [ "$total_memory" != "unknown" ]; then
        if [ "$total_memory" -ge 2 ]; then
...
configure_cloudflare() {
    ...
    prompt_with_default "Enter your Cloudflare API token" "" "CF_API_TOKEN" true
...
configure_route53() {
    ...
    prompt_with_default "Enter your AWS Secret Access Key" "" "AWS_SECRET_ACCESS_KEY" true
...
configure_digitalocean() {
    ...
    prompt_with_default "Enter your DigitalOcean API token" "" "DO_API_TOKEN" true
...
configure_database() {
    ...
    if [ -z "$DB_PASSWORD" ]; then
        if command_exists openssl; then
            ...
            prompt_with_default "Please enter a database password" "" "DB_PASSWORD" true
            if [ -z "$DB_PASSWORD" ]; then
...
configure_email() {
    ...
    # Extract domain suffix for default email
    local domain_suffix=$(echo "$N8N_DOMAIN" | cut -d'.' -f2-)
    local default_email="admin@${domain_suffix}"

    prompt_with_default "Email address for Let's Encrypt" "$default_email" "LETSENCRYPT_EMAIL"
...
generate_encryption_key() {
    ...
    if command_exists openssl; then
        ...
        prompt_with_default "Enter encryption key" "" "N8N_ENCRYPTION_KEY" true
