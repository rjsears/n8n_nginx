...
async def _sync_backup_schedules() -> None:
    ...
    Loads all enabled BackupSchedule records and creates APScheduler jobs for each.
    """
    from api.database import async_session_maker
...
async def _run_scheduled_backup(schedule_id: int) -> None:
    ...
    # Deduplication window - skip if backup ran within this time
    dedup_window = timedelta(minutes=5)

    # Phase 1: Atomic lock-check-update to prevent race conditions
    # Use SELECT FOR UPDATE to lock the schedule row while we check and update last_run
    async with async_session_maker() as db:
        # Lock the schedule row and check if we should run
        result = await db.execute(
        ...
        # Check last_run for deduplication (atomic check while holding lock)
        now = datetime.now(UTC)
        if schedule.last_run and (now - schedule.last_run) < dedup_window:
            logger.warning(
                f"Skipping duplicate scheduled backup {schedule_id} - "
                f"last_run was {(now - schedule.last_run).total_seconds():.0f}s ago "
                f"(within {dedup_window.total_seconds() / 60:.0f} min window)"
            )
            return

        # Update last_run BEFORE running backup (while still holding lock)
        # This prevents other concurrent jobs from passing the dedup check
        schedule.last_run = now
        await db.commit()

        # Store schedule info for phase 2
