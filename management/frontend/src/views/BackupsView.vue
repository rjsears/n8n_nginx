...
import PublicWebsiteRestoreDialog from '../components/backup-ui/PublicWebsiteRestoreDialog.vue'
...
const publicWebsiteRestoreDialog = ref({ open: false, backup: null })
...
<template>
  <div class="space-y-6">
              ...
              >
                ...
                <div v-if="expandedAction[backup.id]" class="border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div v-if="expandedAction[backup.id] === 'restore'" class="p-4 space-y-4">
                    ...
                    <!-- Public Website Section (Collapsible) - Only show if backup has public website files -->
                    <div v-if="backupContents[backup.id]?.public_website_file_count > 0" class="bg-white dark:bg-gray-800 rounded-lg border border-green-200 dark:border-green-700 overflow-hidden">
                      <!-- Section Header (Clickable) -->
                      <button
                        @click="toggleRestoreSection(backup.id, 'public_website')"
                        class="w-full p-4 flex items-center justify-between hover:bg-green-50 dark:hover:bg-green-900/20 transition-colors"
                      >
                        <div class="flex items-center gap-2">
                          <GlobeAltIcon class="h-5 w-5 text-green-600 dark:text-green-400" />
                          <span class="font-semibold text-primary">Public Website Files</span>
                          <span class="text-xs text-secondary bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full">
                            {{ backupContents[backup.id]?.public_website_file_count || 0 }}
                          </span>
                        </div>
                        <ChevronDownIcon
                          :class="[
                            'h-5 w-5 text-gray-400 transition-transform duration-200',
                            expandedRestoreSection[backup.id] === 'public_website' ? 'rotate-180' : ''
                          ]"
                        />
                      </button>

                      <!-- Section Content -->
                      <div v-if="expandedRestoreSection[backup.id] === 'public_website'" class="border-t border-green-100 dark:border-green-800">
                        <div class="p-4 space-y-4">
                          ...
                          <div class="flex items-center justify-between p-4 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
                            <div class="flex items-center gap-3">
                              ...
                              <div>
                                <p class="font-medium text-green-800 dark:text-green-200">
                                  {{ backupContents[backup.id]?.public_website_file_count }} public website files available
                                </p>
                            ...
                            <button
                              @click="publicWebsiteRestoreDialog = { open: true, backup: backup }"
                          ...
                          <!-- File List Preview -->
                          <div v-if="backupContents[backup.id]?.public_website_manifest?.length" class="space-y-2">
                            <h4 class="text-sm font-medium text-secondary">File Preview (first 5 files)</h4>
                            <div
                              v-for="file in backupContents[backup.id].public_website_manifest.slice(0, 5)"
                              :key="file.path"
                            ...
                            >
                              <div class="flex items-center gap-3">
                                <div class="p-2 rounded-lg bg-green-100 dark:bg-green-500/20">
                                  <GlobeAltIcon class="h-4 w-4 text-green-500" />
                                </div>
                                <div>
                            ...
                            <p v-if="backupContents[backup.id].public_website_manifest.length > 5" class="text-sm text-secondary text-center py-2">
                              ... and {{ backupContents[backup.id].public_website_manifest.length - 5 }} more files
                            </p>
                          </div>
                        </div>
                      </div>
    ...
    <PublicWebsiteRestoreDialog
      :open="publicWebsiteRestoreDialog.open"
      :backup="publicWebsiteRestoreDialog.backup"
      @close="publicWebsiteRestoreDialog = { open: false, backup: null }"
      @restored="publicWebsiteRestoreDialog = { open: false, backup: null }; notificationStore.success('Public website files restored successfully')"
    />
  </div>
</template>
