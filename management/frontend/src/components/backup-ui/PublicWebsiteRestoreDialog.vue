          ...
          >
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-3xl max-h-[80vh] flex flex-col border border-gray-400 dark:border-gray-700">
              ...
              <div class="flex-1 overflow-auto p-4">
                ...
                <!-- Image files -->
                <div v-else-if="previewContent.mime_type?.startsWith('image/')" class="flex flex-col items-center gap-4">
                  <img
                    :src="`data:${previewContent.mime_type};base64,${previewContent.content_base64}`"
                    :alt="previewPath"
                    class="max-w-full max-h-[60vh] object-contain rounded-lg shadow-lg"
                  />
                  <p class="text-sm text-gray-500 dark:text-gray-400">
                    {{ previewContent.mime_type }} - {{ formatBytes(previewContent.size) }}
                  </p>
                </div>
                <!-- Other binary files -->
                <div v-else class="text-center text-gray-500 dark:text-gray-400 py-8">
                  <DocumentIcon class="w-16 h-16 mx-auto mb-4 text-gray-300 dark:text-gray-600" />
                  <p class="font-medium">Binary file</p>
                  <p class="text-sm mt-1">{{ previewContent.mime_type }}</p>
                  <p class="text-sm mt-2">Size: {{ formatBytes(previewContent.size) }}</p>
                  <button
                    @click="downloadFile({ path: previewPath })"
