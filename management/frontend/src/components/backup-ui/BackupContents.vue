...
<template>
  <div class="bg-white dark:bg-gray-800 rounded-lg border border-cyan-200 dark:border-cyan-700 overflow-hidden">
    ...
    <div class="p-4 bg-cyan-50 dark:bg-cyan-900/20 border-b border-cyan-200 dark:border-cyan-700">
      ...
      <!-- Summary Stats -->
      <div v-else :class="['grid gap-4', contents.public_website_file_count > 0 ? 'grid-cols-4' : 'grid-cols-3']">
        ...
        <div v-if="contents.public_website_file_count > 0" class="text-center p-3 rounded-lg bg-white dark:bg-gray-800 border border-cyan-100 dark:border-cyan-800">
          <GlobeAltIcon class="h-6 w-6 mx-auto mb-1 text-green-500" />
          <p class="text-2xl font-bold text-primary">{{ contents.public_website_file_count }}</p>
          <p class="text-xs text-secondary">Website Files</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="contents" class="flex border-b border-gray-200 dark:border-gray-700">
      ...
      <button
        v-if="contents.public_website_file_count > 0"
        @click="emit('update:activeTab', 'public_website')"
        :class="[
          'flex-1 px-4 py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors',
    ...
    <div v-if="contents" class="max-h-96 overflow-y-auto">
      ...
      <!-- Public Website Tab -->
      <div v-if="activeTab === 'public_website'">
        <div v-if="!contents.public_website_manifest || contents.public_website_manifest.length === 0" class="p-6 text-center text-secondary">
          <GlobeAltIcon class="h-8 w-8 mx-auto mb-2 text-gray-300 dark:text-gray-600" />
          <p>No public website files metadata available</p>
        </div>
        <table v-else class="w-full">
          <thead class="bg-gray-50 dark:bg-gray-800 sticky top-0">
            <tr>
              ...
              <th class="px-4 py-3 text-left text-xs font-semibold text-secondary uppercase tracking-wide">Path</th>
              <th class="px-4 py-3 text-right text-xs font-semibold text-secondary uppercase tracking-wide">Size</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-secondary uppercase tracking-wide">Modified</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            <tr v-for="(file, index) in contents.public_website_manifest" :key="index" class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
              <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                  <GlobeAltIcon class="h-4 w-4 text-green-500 flex-shrink-0" />
                  <span class="font-medium text-primary">{{ file.name || 'Unknown' }}</span>
                </div>
              </td>
              <td class="px-4 py-3">
                <code class="text-xs bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded font-mono text-secondary truncate max-w-xs block" :title="file.path">{{ file.path || 'N/A' }}</code>
